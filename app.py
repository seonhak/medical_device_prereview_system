        ...
        if keyword == '외형':
            ...
            if len(shape_file) == 0:
                ...
                if(type(shape_error) != type(None) and len(shape_error) != 0):
                    error_messages.append(f"[ 외형파일에 검토사항이 검출되었습니다. 검토사항 검출 개수 : {len(shape_error)} ] ")
                    error_messages.append(f"모양 및 구조 - 규정 제 9조")
            ...
            if len(size_file) == 0:
                ...
                #     if label == 1:
                #         error_messages.append('[ 치수파일에 검토사항이 검출되었습니다. ]')
            ...
            if len(mat_file) == 0:
                ...
                if(type(mat_error) != type(None) and len(mat_error) != 0):
                    error_messages.append(f"[ 원재료파일에 검토사항이 검출되었습니다. 검토사항 검출 개수 : {len(mat_error)} ] ")
                    error_messages.append(f"원재료 - 규정 제 10조")
            ...
            if len(wp_file) == 0:
                ...
                if(type(wp_error) != type(None) and len(wp_error) != 0):
                    error_messages.append(f"[ 작용원리파일에 검토사항이 검출되었습니다. 검토사항 검출 개수 : {len(wp_error)} ] ")
                    error_messages.append(f"모양 및 구조 - 규정 제 9조")
            ...
            if len(usage_file) == 0:
                ...
                if(type(usage_error) != type(None) and len(usage_error) != 0):
                    error_messages.append(f"[ 사용방법파일에 검토사항이 검출되었습니다. 검토사항 검출 개수 : {len(usage_error)} ] ")
                    error_messages.append(f"사용방법 - 규정 제 13조")
            ...
            if len(pfu_file) == 0 :
                ...
                if(type(pfu_error) != type(None) and len(pfu_error) != 0):
                    error_messages.append(f"[ 주의사항파일에 검토사항이 검출되었습니다. 검토사항 개수 : {len(pfu_error)} ] ")
                    error_messages.append(f"사용 시 주의사항 - 규정 제 14조")
...
folder_path = r"C:\Users\USER\Desktop\식약처\고도화\고도화용 데이터 53건\검증용 데이터 53건\test_folder"
...
for folder in folder_list:
    ...
    if error_result:
        ...
        save_filepath = folder + fr"/report{num}_no_error.hwp"
